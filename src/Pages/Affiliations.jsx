...
export default function Affiliations() {
  ...
  const fetchAffiliations = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_URL}/api/affiliation`);
      if (!res.ok) {
        let errMsg = 'Failed to fetch affiliations';
  ...
  const handleSave = async (e) => {
    ...
    try {
      let res;
      if (editItem) {
        res = await fetch(`${API_URL}/api/affiliation/${editItem._id}`, {
          method: 'PUT',
          body: formData,
        });
      } else {
        res = await fetch(`${API_URL}/api/affiliation/upload`, {
          method: 'POST',
          body: formData,
        });
      }
      ...
      if (!res.ok) {
        let errMsg = 'Failed to save affiliations';
        const contentType = res.headers.get('content-type');
        if (contentType && contentType.includes('application/json')) {
          const errJson = await res.json();
          errMsg = errJson.message || errMsg;
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/affiliation/${id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
        let errMsg = 'Failed to delete affiliations';
        const contentType = res.headers.get('content-type');
        if (contentType && contentType.includes('application/json')) {
          const errJson = await res.json();
          errMsg = errJson.message || errMsg;
    ...
    <div>
      <div className="page-header">
        <div className="page-header-left">
          <h2>Affiliations</h2>
          <p>Publish, view, and manage affiliation documents in PDF format.</p>
        </div>
        <button className="btn-primary" onClick={openAdd} disabled={loading}>
          <MdAdd /> Upload Affiliation
        </button>
      </div>
      ...
      <div className="filters-bar">
        ...
        <span className="filter-count">
          {filtered.length} of {data.length} affiliations
        </span>
      </div>
      ...
      {error && (
        <div style={{ padding: '12px 16px', background: 'var(--danger-light)', color: 'var(--danger)', borderRadius: 8, marginBottom: 16, fontSize: 13, fontWeight: 500 }}>
          ⚠️ Error loading affiliations: {error}. Please verify the backend is running.
        </div>
      )}
              ...
              {loading && data.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ textAlign: 'center', padding: '30px' }}>
                    <div style={{ color: 'var(--text-secondary)' }}>Loading affiliations...</div>
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
      ...
      <Modal
        open={modal}
        onClose={closeModal}
        title={editItem ? 'Edit Affiliation' : 'Upload Affiliation'}
        subtitle="Publish a affiliation PDF on the site"
        icon={<MdBook />}
        iconBg="#dbeafe"
        iconColor="#2563eb"
        footer={
          <>
            ...
            <button className="btn-primary" onClick={handleSave} disabled={loading}>
              {loading ? 'Saving...' : editItem ? 'Update Affiliation' : 'Upload Affiliation'}
            </button>
          </>
        }
      >
        <div className="form-grid cols-1">
          <div className="form-group">
            <label className="form-label">Affiliation Title <span className="required">*</span></label>
            <input
              className="form-input"
              placeholder="e.g. University Affiliation 2026 - 2027"
              value={form.title}
      ...
      >
        <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>
          Are you sure you want to delete this affiliations? The PDF file and database record will be permanently removed.
        </p>
      </Modal>
    </div>
  );
